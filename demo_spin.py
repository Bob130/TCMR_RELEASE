...
def main(args):
    ...
    output_path = osp.join('./output/demo_spin_output', os.path.basename(video_file).replace('.mp4', ''))
    ...
    print("\nRunning SPIN on each person tracklet...")
    tcmr_time = time.time()
    tcmr_results = {}
    for person_id in tqdm(list(tracking_results.keys())):
        ...
        crop_dataloader = DataLoader(dataset, batch_size=256, num_workers=1)  # works=1 让bboxes中batch最后一个返回最新的bboxes
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(crop_dataloader):
                ...
                pred_pose.append(spin_output['theta'][:, 3:75])
                pred_betas.append(spin_output['theta'][:, 75:])
                pred_joints3d.append(spin_output['kp_3d'])
